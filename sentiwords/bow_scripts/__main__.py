...
def create_embedding_files(train,
                           devel,
                           test,
                           embeddings,
                           basepath='experiment'):
...
def create_BoW(train,
               devel,
               test,
               cbs=[10, 20, 50, 100, 200, 500, 1000, 2000],
               no_vectors=[1, 10, 25, 50, 100, 200, 500, 1000],
               BoW_folder='BoW'):
    """
    ...
    for size in cbs:
        ...
        for a in no_vectors:
            ...
            print('Creating BoWs for codebook size {} and a={} ...'.format(
                str(size), str(a)))
            subprocess.run(
                [
                    "java", "-Xmx15g", "-jar", __OPEN_XBOW_PATH, "-i", train,
                    "-o",
                    join(output_directory, basename(train)), "-B",
                    join(output_directory, 'codebook'), "-size",
                    str(size), "-a",
                    str(a), "-log", "-norm", '1', "-writeName"
                ]
            )  # log: logarithmic term weighting, norm 1: normalize bag by its length, writeName: write tweet ID into output
            for eval_file in [devel, test]:
                subprocess.run(
                    [
                        "java", "-Xmx15g", "-jar", __OPEN_XBOW_PATH, "-i",
                        eval_file, "-o",
                        join(output_directory, basename(eval_file)), "-b",
                        join(output_directory,
                             'codebook'), "-norm", '1', "-writeName"
                    ]
                )  # norm 1: normalize bag by its length, writeName: write tweet ID into output
...
def evaluate_BoW(train, devel, test, complexity, BoW_folder='BoW'):
    ...
    """
    size_folders = [
        join(BoW_folder, folder) for folder in listdir(BoW_folder)
        if isdir(join(BoW_folder, folder))
    ]
    for size in size_folders:
        a_folders = [
            join(size, folder) for folder in listdir(size)
            if isdir(join(size, folder))
        ]
        for a in a_folders:
            print('Evaluating BoWs for codebook size {} and a={} ...'.format(
                basename(size), basename(a)))
            run_SVM(
                join(a, train),
                join(a, devel),
                join(a, test),
                complexity=complexity,
                cm_path=join(a, 'confusion_matrix.pdf'),
                output=join(a, 'results.csv'))
...
def get_best_results(file_path, n_best=1):
    ...
    df[['Accuracy Development',
        'Accuracy Test']] = df[['Accuracy Development',
                                'Accuracy Test']].applymap(f)
    df[['Accuracy Development', 'Accuracy Test']] = df[[
        'Accuracy Development', 'Accuracy Test'
    ]].astype(float)  # applymap: apply f on all elements of selected df

    best_results = []
    for idx in range(n_best):
        best_result = df.loc[df['Accuracy Development'].idxmax(
        )]  # line (of df) where Accuracy Development has maximum
        best_results.append((best_result['Complexity'],
                             best_result['Accuracy Development'].astype(str),
                             best_result['Accuracy Test'].astype(str)))
        df = df[df['Complexity'] !=
                best_result['Complexity']]  # remove this line from df
    return best_results
...
def aggregate_performance(basepath):
    ...
    for size in sizes:
        for assignment_value in folder_dict[size]:
            ...
            best_results = get_best_results(
                ...
                n_best=2)
            best_overall_result = (
                size, assignment_value, best_results[0][0], best_results[0][1],
                best_results[0][2]) if float(best_results[0][1]) > float(
                    best_overall_result[3]) else best_overall_result
    ...
    df = pd.DataFrame.from_dict(
        data, orient='index'
    ).T  # T: transpose (table with size in rows and a in columns)
...
def main():
    ...
    parser.add_argument(
        '-embeddings', required=True, help='csv file with word embeddings')
    args = vars(parser.parse_args())
    train_tweets, devel_tweets, test_tweets, output = args['input'][0], args[
        'input'][1], args['input'][2], args['output']
    create_embedding_files(train_tweets, devel_tweets, test_tweets,
                           args['embeddings'], output)
    train, devel, test = join(output, basename(train_tweets)), join(
        output, basename(devel_tweets)), join(output, basename(test_tweets))
    create_BoW(
        train,
        devel,
        test,
        ...
        BoW_folder=join(args['output'], 'BoW'))
    evaluate_BoW(
        basename(train),
        basename(devel),
        basename(test),
        complexity=args['C'],
        BoW_folder=join(output, 'BoW'))
